...
// app/infrastructure/http/index.ts
...
function errorHandler(err, _req, res, next) {
  ...
  if (err instanceof import_zod2.default.ZodError) {
...
var UserSchemas = class {
  ...
  static emailSchema = import_zod4.z.string().email("Invalid email format").min(1, "Email is required").max(255, "Email cannot exceed 255 characters");
  ...
  static userIdSchema = import_zod4.z.number().int("User ID must be an integer").positive("User ID must be positive");
  static passwordSchema = import_zod4.z.string().min(8, "Password must be at least 8 characters").regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/, "Password must contain uppercase, lowercase, number and special character");
  static createUserSchema = import_zod4.z.object({
    name: this.nameSchema,
    email: this.emailSchema,
    passwordHash: import_zod4.z.string().min(8, "Password hash is required")
  });
  static userEntitySchema = import_zod4.z.object({
...
var AccountSchemas = class {
  ...
  static nameSchema = import_zod5.z.string().min(2, "Name must have at least 2 characters").max(100, "Name cannot exceed 100 characters").regex(/^[a-zA-ZÀ-ÿ\s]+$/, "Name can only contain letters and spaces");
  static accountIdSchema = import_zod5.z.number().int("Account ID must be an integer").positive("Account ID must be positive");
  static accountTypeSchema = import_zod5.z.nativeEnum(AccountType);
  static balanceSchema = import_zod5.z.number().refine((val) => Number.isFinite(val), {
    message: "Balance must be a finite number"
  });
  static createAccountSchema = import_zod5.z.object({
    type: this.accountTypeSchema,
    balance: this.balanceSchema,
    userId: UserSchemas.userIdSchema,
    bankName: BankSchemas.nameSchema
  });
  static accountEntitySchema = import_zod5.z.object({
...
var TransactionSchemas = class {
  ...
  static transactionIdSchema = import_zod6.z.number().int("Transaction ID must be an integer").positive("Transaction ID must be positive");
  static amountSchema = import_zod6.z.number().refine((val) => Number.isFinite(val), {
    message: "Amount must be a finite number"
  });
  static descriptionSchema = import_zod6.z.string().min(2, "Description must have at least 2 characters").max(255, "Description cannot exceed 255 characters");
  static accountIdSchema = AccountSchemas.accountIdSchema;
  static transactionDateSchema = import_zod6.z.date();
  static transactionTypeSchema = import_zod6.z.nativeEnum(TransactionType);
  static createTransactionSchema = import_zod6.z.object({
  ...
  static transactionEntitySchema = import_zod6.z.object({
...
var TypeOrmTransactionRepository = class {
  ...
  async findByDateRange(startDate, endDate) {
    const transactions = await this.repository.find({
      where: {
        transactionDate: (0, import_typeorm8.Between)(startDate, endDate)
      },
      relations: [
        "sourceAccount",
        "destinationAccount"
...
var DeleteAccountOfUserSchemas = class {
  ...
  static requestSchema = import_zod9.z.object({
    accountId: AccountSchemas.accountIdSchema
  });
  static httpRequestSchema = import_zod9.z.object({
...
var GetAllAccountsOfUserSchemas = class {
  ...
  static requestSchema = import_zod10.z.object({
    userId: UserSchemas.userIdSchema
  });
  static httpRequestSchema = import_zod10.z.object({
    params: this.requestSchema
  });
};
...
var UpdateAccountOfUserSchemas = class {
  ...
  static requestSchema = import_zod11.z.object({
    accountId: AccountSchemas.accountIdSchema,
    account: import_zod11.z.object({
      type: AccountSchemas.accountTypeSchema.optional()
    })
  });
  static httpRequestSchema = import_zod11.z.object({
...
var AddTransactionToUserSchemas = class {
  ...
  static requestSchema = import_zod12.z.object({
...
var GetAllOfUserSchemas = class {
  ...
  static requestSchema = import_zod13.default.object({
    userId: UserSchemas.userIdSchema
  });
  static httpRequestSchema = import_zod13.default.object({
    params: this.requestSchema
  });
};
...
var CreateUserSchemas = class {
  ...
  static requestSchema = import_zod14.z.object({
    account: import_zod14.z.object({
      type: AccountSchemas.accountTypeSchema,
      balance: AccountSchemas.balanceSchema
    }),
    bank: import_zod14.z.object({
  ...
  static httpRequestSchema = import_zod14.z.object({
    body: this.requestSchema
  });
};
...
var DeleteUserSchemas = class {
  ...
  static requestSchema = import_zod15.z.object({
    id: UserSchemas.userIdSchema
  });
  static httpRequestSchema = import_zod15.z.object({
    params: this.requestSchema
  });
};
...
var UserLoginSchemas = class {
  ...
  static requestSchema = import_zod16.z.object({
    email: UserSchemas.emailSchema,
    password: import_zod16.z.string().min(8).max(100)
  });
  static httpRequestSchema = import_zod16.z.object({
    body: this.requestSchema
  });
};
...
var UpdateUserSchemas = class {
  ...
  static requestSchema = import_zod17.z.object({
  ...
  static httpRequestSchema = import_zod17.z.object({
    body: this.requestSchema,
    params: import_zod17.z.object({
      id: UserSchemas.userIdSchema
    })
...
var GetAllOfUserUseCase = class {
  ...
  handleError(error) {
    if (error instanceof import_zod22.ZodError) {
      const errors = error.errors.map((err) => `${err.path.join(".")}: ${err.message}`);
...
var CreateUserUseCase = class {
  ...
  handleError(error) {
    if (error instanceof import_zod23.ZodError) {
      const errors = error.errors.map((err) => `${err.path.join(".")}: ${err.message}`);
...
var UpdateUserUseCase = class {
  ...
  handleError(error) {
    if (error instanceof import_zod26.ZodError) {
      const errors = error.errors.map((err) => `${err.path.join(".")}: ${err.message}`);
...
var AuthService = class {
  ...
  async hashPassword(raw) {
    return import_bcryptjs2.default.hash(raw, 10);
  }
  async comparePassword(raw, hash) {
    return import_bcryptjs2.default.compare(raw, hash);
  }
...
// app/infrastructure/http/server/index.ts
...
var Server = class {
  ...
  constructor(config) {
    ...
    this.router = import_express2.default.Router();
  }
  async init() {
    try {
  ...
  async initializeDatabase() {
    try {
      ...
      logWithContext.error("Failed to connect to database", error);
  ...
  async start() {
    try {
      await this.init();
      this.httpServer = this.app.listen(this.config.port, this.config.address, () => {
  ...
  setupGracefulShutdown() {
    const gracefulShutdown = /* @__PURE__ */ __name((signal) => {
      if (this.isShuttingDown) {
        logWithContext.server(`Received ${signal} during shutdown, ignoring...`);
        return;
      }
  ...
  setupMiddlewares() {
    this.app.use((0, import_morgan.default)("combined", {
      stream: morganStream
    }));
