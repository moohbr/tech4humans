...
var UserSchemas = class {
  ...
  static emailSchema = z3.string().email("Invalid email format").min(1, "Email is required").max(255, "Email cannot exceed 255 characters");
  ...
  static userIdSchema = z3.number().int("User ID must be an integer").positive("User ID must be positive");
  static passwordSchema = z3.string().min(8, "Password must be at least 8 characters").regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/, "Password must contain uppercase, lowercase, number and special character");
  static createUserSchema = z3.object({
    name: this.nameSchema,
    email: this.emailSchema,
    passwordHash: z3.string().min(8, "Password hash is required")
  });
  static userEntitySchema = z3.object({
