...
// app/domain/use-cases/account/add-to-a-user/index.spec.ts
...
describe("AddAccountToUserUseCase", () => {
  const useCase = new AddAccountToUserUseCase(mockAccountRepository, mockUserRepository);
  const userId = UserId.createUnsafe(1);
  const accountType = AccountTypeVO.create("Poupan\xE7a");
  const balance = AccountBalance.create(1e3);
  const bankName = BankName.create("Bank of Jest");
  const request = new AddAccountToUserRequest(userId, accountType, balance, bankName);
  const fakeAccount = AccountEntity.create(accountType.getValue(), balance.getValue(), userId.getValue(), bankName.getValue());
  beforeEach(() => {
    jest.clearAllMocks();
  });
